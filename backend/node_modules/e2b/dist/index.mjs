...
var DEFAULT_ERROR_MAP = {
  ...
  [Code.ResourceExhausted]: (message) => new RateLimitError(
    `${message}: Rate limit exceeded, please try again later.`
  ),
  [Code.Unavailable]: formatSandboxTimeoutError,
  [Code.Canceled]: (message) => new TimeoutError(
...
var Filesystem2 = class {
  ...
  async read(path2, opts) {
    ...
    const res = await this.envdApi.api.GET("/files", {
      ...
      throw await handleEnvdApiFetchError(err2, this.checkHealth);
    });
  ...
  async write(pathOrFiles, dataOrOpts, opts) {
    ...
    const hasStreamableData = runtime !== "browser" && writeFiles.some((file) => file.data instanceof ReadableStream);
    const useOctetStream = (((_a3 = writeOpts == null ? void 0 : writeOpts.useOctetStream) != null ? _a3 : hasStreamableData) || useGzip) && supportsOctetStream;
    ...
    if (useOctetStream) {
      ...
      const uploadResults = await Promise.all(
        writeFiles.map(async (file) => {
          ...
          const signal = isStream ? writeOpts == null ? void 0 : writeOpts.signal : this.connectionConfig.getSignal(
            writeOpts == null ? void 0 : writeOpts.requestTimeoutMs,
            writeOpts == null ? void 0 : writeOpts.signal
          );
          const res = await this.envdApi.api.POST("/files", __spreadValues({
            params: {
              query: {
                path: filePath,
                username: user
  ...
  async watchDir(path2, onEvent, opts) {
    ...
    if ((opts == null ? void 0 : opts.allowNetworkMounts) && this.envdApi.version && compareVersions2(this.envdApi.version, ENVD_VERSION_WATCH_NETWORK_MOUNTS) < 0) {
      throw new TemplateError(
    ...
    const events = this.rpc.watchDir(
      {
        ...
        allowNetworkMounts: (_d = opts == null ? void 0 : opts.allowNetworkMounts) != null ? _d : false
      },
      {
        ...
        signal: controller.signal,
        timeoutMs: (_e = opts == null ? void 0 : opts.timeoutMs) != null ? _e : this.defaultWatchTimeout
      }
    );
    try {
  ...
  iterateEvents() {
    return __asyncGenerator(this, null, function* () {
      ...
      try {
        ...
        yield* __yieldStar(this.flushDecoders());
        throw e;
      }
...
var VolumeConnectionConfig = class _VolumeConnectionConfig {
  constructor(volume, opts) {
    ...
    this.logger = opts == null ? void 0 : opts.logger;
    this.requestTimeoutMs = (_c = opts == null ? void 0 : opts.requestTimeoutMs) != null ? _c : REQUEST_TIMEOUT_MS2;
...
var Volume = class _Volume {
  ...
  async readFile(path2, opts) {
    ...
    if (format === "stream") {
      ...
      try {
        const res2 = await client.api.GET("/volumecontent/{volumeID}/file", {
          params: {
        ...
        const err2 = handleApiError(res2, VolumeError);
        if (err2) {
          if (res2.response.body && !res2.response.bodyUsed) {
            await res2.response.body.cancel().catch(() => {
            });
        ...
        return wrapStreamWithConnectionCleanup(
          res2.data,
          {
            ...
            cleanup,
            controller,
            idleTimeoutMs: (_c = opts == null ? void 0 : opts.streamIdleTimeoutMs) != null ? _c : config.requestTimeoutMs
          }
        );
      } catch (err2) {
        cleanup();
    ...
    if (format === "bytes") {
      return res.data instanceof ArrayBuffer ? new Uint8Array(res.data) : new Uint8Array();
    }
    if (format === "text") {
  ...
  async writeFile(path2, data, opts) {
    ...
    const isStream = body instanceof ReadableStream;
    const signal = isStream ? opts == null ? void 0 : opts.signal : config.getSignal();
    const res = await client.api.PUT("/volumecontent/{volumeID}/file", __spreadValues({
      params: {
        path: {
          volumeID: this.volumeId
        },
...
var Pty = class {
  ...
  async create(opts) {
    ...
    const requestTimeoutMs = (_a3 = opts == null ? void 0 : opts.requestTimeoutMs) != null ? _a3 : this.connectionConfig.requestTimeoutMs;
    const envs = __spreadValues({}, (_b = opts == null ? void 0 : opts.envs) != null ? _b : {});
...
var SandboxApi = class {
  ...
  static async pause(sandboxId, opts) {
    ...
    const res = await client.api.POST("/sandboxes/{sandboxID}/pause", {
      ...
      body: {
        memory: (_a3 = opts == null ? void 0 : opts.keepMemory) != null ? _a3 : true
      },
      signal: config.getSignal(opts == null ? void 0 : opts.requestTimeoutMs, opts == null ? void 0 : opts.signal)
    });
  ...
  static async createSandbox(template, timeoutMs, opts) {
    ...
    const keepMemory = typeof onTimeout !== "string" && "keepMemory" in onTimeout ? (_c = onTimeout.keepMemory) != null ? _c : true : true;
    const autoResume = (_e = (_d = opts == null ? void 0 : opts.lifecycle) == null ? void 0 : _d.autoResume) != null ? _e : false;
    ...
    const body = {
      ...
      autoPauseMemory: action === "pause" ? keepMemory : void 0,
...
async function getAllFilesInPath(src, contextPath, ignorePatterns, includeDirectories = true) {
  ...
  return Array.from(files.values()).sort(
    (a, b) => a.fullpath() < b.fullpath() ? -1 : a.fullpath() > b.fullpath() ? 1 : 0
  );
}
...
async function waitForBuildFinish(client, {
  ...
  const pollStatus = async () => {
    ...
    buildStatus.logEntries.forEach((logEntry) => onBuildLogs == null ? void 0 : onBuildLogs(logEntry));
    return buildStatus;
  };
...
var TemplateBase = class _TemplateBase {
  ...
  gitClone(url, path2, options) {
    ...
    if (path2) {
      args.push(shellQuote(path2.toString()));
    }
    return this.runInNewStackTraceContext(
      () => this.runCmd(args.join(" "), { user: options == null ? void 0 : options.user })
    );
